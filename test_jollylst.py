...
def f06(string1, string2):
    ...
    s2 = string2
    if len(s1) >= len(s2):
        print("'{a}' is longer than '{b}' by {c} chars".format(a=s1, b=s2, c=len(s1)-len(s2)))
        print("'{a}' has only {b}% the number of chars of '{c}'".format(a=s2, b=float(len(s2)/len(s1)*100), c=s1))
    else:
        print("'{a}' is longer than '{b}' by {c} chars".format(a=s2, b=s1, c=len(s2)-len(s1)))
        print("'{a}' has only {b}% the number of chars of '{c}'".format(a=s1, b=float(len(s1)/len(s2)*100), c=s2))
...
###############################################################################
def f09():
    Sum = sum(n for n in range(500) if n % 3 == 0 or n % 5 == 0)
    return Sum
